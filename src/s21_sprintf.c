...
void itoa(write_in_buf *output, long double n, char s[], int itsFloat) {
  ...
  if(((origin_n_doub < 1 && origin_n_doub >= 0) || (origin_n_doub > -1 && origin_n_doub <= 0)) && output->accuracy != 0 && itsFloat){
   s[i++] = '.';
   s[i++] = '0';
  }
  s[i] = '\0';
  ...
  //   i=0;
  //   }
  if (origin_n_doub >= 0 && output->flag_space && !output->flag_zero && !output->flag_plus && !output->flag_u) s[i++] = ' ';
  if (origin_n_doub >= 0 && output->flag_space && output->flag_zero && !output->flag_plus && !output->flag_u) output->flag_add_space=1;

  if (origin_n_doub < 0 && !output->flag_u && !output->flag_zero) s[i++] = '-';
  if(origin_n_doub < 0 && output->flag_zero) output->flag_negative =1;

  if (origin_n_doub >= 0 && output->flag_plus  && !output->flag_u && !output->flag_zero) s[i++] = '+';
  if (origin_n_doub >= 0 && output->flag_plus && output->flag_zero) output->flag_positiv =1;
...
void add_sign (write_in_buf *output, char *buf){
    if( output->flag_negative) {
    buf[output->index_buf_mass++] = '-';
    output->res++;
    output->space--;
  }
  if( output->flag_positiv) {
    buf[output->index_buf_mass++] = '+';
    output->res++;
    output->space--;
  }
  if( output->flag_add_space) {
    buf[output->index_buf_mass++] = ' ';
    output->res++;
    output->space--;
  }
